...
def test_manual_snapshot_path_text_preserves_cli_output() -> None:
    ...
    assert manual_snapshot_fixture_creation_next_action(
        input_path=input_path,
        history_path=None,
    ) == (
        "Next: wq currency-exchange-manual-snapshot "
        f"--input-path {input_path} --history-path <previous-snapshot> "
    ...
    assert manual_snapshot_fixture_creation_next_action(
        ...
        "Next: wq currency-exchange-manual-snapshot "
        f"--input-path {input_path} --history-path {history_path} "
        "--output-fixture-path <fixture-output>"
    )
