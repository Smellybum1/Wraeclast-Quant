...
Current project preflight has two automation-eligible API resources after human review evidence and manual `RESOURCES.md` approval: `poe_ninja_poe2_currency` and `official_currency_exchange_api`. The official Currency Exchange API is auth-approved for local planning/dry-runs but still blocked for live collection until OAuth/app-registration runtime handling, user-agent/contact configuration, credential storage outside the repo, cache/backoff behavior, and failure-closed tests are implemented. Conditional values such as `manual-or-api-if-available`, `manual-review-or-api-if-available`, and `api-or-manual-review` remain blocked until a human records source evidence and manually changes `RESOURCES.md` to an explicit approved value such as `api`, `rss`, or `download`.
