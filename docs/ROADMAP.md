...
- Use `wq currency-exchange-manual-snapshot --input-path examples/pathofexile_currency_exchange_manual_snapshot_template.json` to validate local manual observations; validation-only output points to rerunning with `--history-path` and `--output-fixture-path` when creating a local preview connector fixture. Successful fixture writes now print the matching `wq connector-fixture-export --review-path examples/reviews/pathofexile_currency_exchange_connector_review.json --fixture-path <fixture-output> --output-path <manual-import-output>` next action.
...
- Review queue guidance now uses run-scoped worksheet and outcome-decision paths like `data/processed/review_queue_run_<id>.md` and `data/processed/outcome_decisions_run_<id>.json`, preserving old filled review files while keeping the next run's batch review path clear.
...
- `wq review-queue` and `wq review-coverage` now preserve non-default `--database-path` values in printed batch-review, saved worksheet, and one-off outcome command templates.
...
- `wq watchlist` now points fully reviewed stored runs with local calibration review prompts back to `wq calibration`, keeping the daily recommendation surface connected to read-only outcome feedback without changing scoring.
...
- Successful manual-import validation now prints the matching `wq daily --input-path <file>` next action, keeping the fresh local observation path moving from read-only validation into the next manual daily run.
- Manual-import diagnostics now point back to `wq validate-import --input-path <file>` after read-only inspection, keeping score/signal review connected to the safe validation-before-daily path.
...
- Outcome report-written output is split into focused rendering helpers, matching calibration report output structure while preserving CLI output.
...
- Local status now includes a read-only `Calibration prompts` row that points reviewed runs with prompt patterns back to `wq calibration` without failing strict mode or changing public artifacts.
...
- Currency Exchange UI observation signal and ratio-math helpers are split behind the existing collector facade, preserving conservative manual-import scoring behavior.
- Currency Exchange UI observation manual-import payload conversion is split behind the existing collector facade, preserving item names, signal JSON, and CLI conversion behavior.
- Currency Exchange UI observation file I/O and manual-import writer orchestration are split behind the existing collector facade, preserving CLI conversion and review-sidecar behavior.
- Currency Exchange UI observation export output now prints the later `review-queue --context-path` handoff when a local review sidecar is written, keeping sidecar notes connected to post-daily outcome review.
