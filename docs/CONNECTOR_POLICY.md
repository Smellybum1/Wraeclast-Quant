...
Use `wq currency-exchange-ui-observation --input-path <file> --output-path <file>` for manually transcribed in-game Currency Exchange ratio and stock-ladder rows when API-shaped hourly fields are unavailable. The command reads only local JSON, writes manual-import-compatible derived signals, and does not approve live collection, reuse browser sessions, read credentials, exchange tokens, fetch from the trade site or API, scrape or OCR screenshots, create raw caches, write database snapshots, or publish artifacts.
